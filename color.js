...
d3.json("http://10.200.1.75:8012/bn?name=bncancer1", function(error, json) { //"http://10.200.1.75:8012/bn?name=bncancer1,http://10.200.1.75:8012/bn?name=bnlung1 Dgraph.json"
    ...
    var leftContainer = d3.select("body").append("svg")
                                        .attr("width", width)
                                        .attr("height", height);
                                        leftContainer.attr("id", "leftContainer").append("rect").attr("x", 10).attr("y", 10).attr("height", height-20).attr("width", width-20).style("fill", "white").style("stroke", "#575757").style("stroke-width", "5").attr("rx", 20).attr("ry", 20);
    ...
    var rectAttributes = rects.attr("x", 0)
                              ...
                              .style("stroke", function(d,i) {if(d.properties.type == "diagnosis") {return "#E8440C";};
                                                             if(d.properties.type == "therapy") {return "#E8D20E";};
                                                             if(d.properties.type == "examination") {return "#FFA000";};
                                                             if(d.properties.type == "symptom") {return "#FF0077";}})
    ...
    var nameAttributes = name.style("fill", "#575757")
                     .attr("x", 5)
                     .attr("y", 23)
    ...
    .range(["#0ABE31", "#0ABCC8", "#0AC86A", "#0A83BE", "#00B18C", "#d0743c", "#ff8c00"]);
    //RadioButtons
...
//ANPASSEN UND NACHGUCKEN
    ...
    buttonGroups.append("text")
                ...
                .on("click", function highlightButton(){
                    alert(this.id)//parents = getParentsIndex()
                    highlightNode(this.id.split(" ")[0],true,parents);
                    active = false;
...
rightContainer.selectAll("g").append("rect").attr("x", 20).attr("y", function(d,i){return i *200}).attr("height", 300).attr("width", widthRight-25).style("fill", "white").style("stroke", "#575757").style("stroke-width", "5").attr("rx", 15).attr("ry", 15);
...
var rectLegende = legendeGroup
                ...
                .style("stroke", function(d,i) {if(i==1) {return "#E8D20E"}
                                                else if(i==2) {return "#FFA000"}
                                                else if(i==3) {return "#E8440C"}
                                                else if(i==4) {return "#FF0077"}})
...
function calculateTableContent(indexOfNode){
    ...
    function tabulate(rows, columns, parentSize) {
        ...
        ths.style("border-right", function(d,i){
            if(i == parentSize){return "solid #FE642E";}}) 
    ...
    function highlightNode(id,ac=false,parents){
        for (i=0; i<json.nodes.length; i++){
              if (id == json.nodes[i].name){
                  if (activeNodes[i] && ac){
                     d3.select(document.getElementById(id).firstChild).style("stroke-width", 3);
